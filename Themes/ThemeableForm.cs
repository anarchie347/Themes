...
namespace Anarchie.Themes
{
	...
	public class ThemeableForm<ThemeType> : Form where ThemeType : Theme
	{
		...
		private static void ValidateTypesOfThemeableProperties(PropertyInfo themeProperty, PropertyInfo editingProperty, IThemeableControl ctrl)
		{
			...
			//The below two if statements were generated by ChatGPT. I sort of understand them, but they check that themePropertyType and editingPropertyType are Func<> and Action<> with any amount of parameters
            ...
            {
				throw new Exception($"{themeProperty.Name} in class {ctrlType.Name} was of type {themePropertyType.Name}, when it should have been an implementation of Func<>");
			}
			MessageBox.Show(themePropertyType.ToString());
			MessageBox.Show(editingPropertyType.ToString());
            MessageBox.Show((typeof(Func<>) == themePropertyType.GetGenericTypeDefinition()).ToString());
            MessageBox.Show((typeof(Action<>) == editingPropertyType.GetGenericTypeDefinition()).ToString());
            MessageBox.Show((typeof(Action<,>) == editingPropertyType.GetGenericTypeDefinition()).ToString());

			//PROBLEM FOUND...
			//Problem is that editingPropertyType has 2 generic args
            ...
            {
                throw new Exception($"{editingProperty.Name} in class {ctrlType.Name} was of type {editingPropertyType}, when it should have been an implementation of Action<>");
            }
			...
			if (!(editingPropertyThemeableType == ctrlType))
				throw new Exception($"The first generic arguement for {editingProperty.Name} was {editingPropertyThemeableType.Name} whcih did not correspond to the type of the class which was {ctrlType.Name}");
